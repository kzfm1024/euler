...
(define (products-4-adjacent-numbers grid i j)
  (define (product-left)
    (let ((N (lambda (i j) (grid-ref grid i j))))
      (if (< (+ j 4) 20)
          (* (N i j)
             (N i (+ j 1))
             (N i (+ j 2))
    ...
    (let ((N (lambda (i j) (grid-ref grid i j))))
      (if (< (+ i 4) 20)
          (* (N i j)
             ...
             (N (+ i 3) j))
          -1)))
  (define (product-down-left)
    (let ((N (lambda (i j) (grid-ref grid i j))))
      (if (and (< (+ i 4) 20) (< (+ j 4) 20))
          (* (N i j)
             (N (+ i 1) (+ j 1))
             (N (+ i 2) (+ j 2))
             (N (+ i 3) (+ j 3)))
          -1)))
  (define (product-down-right)
    (let ((N (lambda (i j) (grid-ref grid i j))))
      (if (and (< (+ i 4) 20) (> (- j 4) 0))
          (* (N i j)
             (N (+ i 1) (- j 1))
...
(apply max
  (map2 (i in (iota 20))
    (apply max         
      (map2 (j in (iota 20))
        (apply max (products-4-adjacent-numbers the-grid i j))))))
;=> 70600674           
